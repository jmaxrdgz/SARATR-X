...
class MGF(nn.Module):
    ...
    def _create_and_register_kernels(self, k, scale_idx):
        ...
        def _create_gauss_kernel(r):
            ...
            return (
                torch.from_numpy((M_13)).float(), 
                torch.from_numpy((M_23)).float(), 
                torch.from_numpy((M_11)).float(), 
                torch.from_numpy((M_21)).float()
            )
        
        M13, M23, M11, M21 = _create_gauss_kernel(k)
        
        weight_x1 = M11.view(1, 1, k*2+1, k*2+1)
        weight_x2 = M21.view(1, 1, k*2+1, k*2+1)
        weight_y1 = M13.view(1, 1, k*2+1, k*2+1)
        weight_y2 = M23.view(1, 1, k*2+1, k*2+1)
    ...
    def forward(self, x):
        ...
        Returns:
            Tensor: List of scale gradient features list([B, 1, H, W])
        """
        assert torch.min(x) >= 0, "MGF requires non-negative input"
    ...
    def _compute_single_scale(self, x, kensize, weight_x1, weight_x2, 
        ...
        x = F.pad(x, pad=(k, k, k, k), mode="reflect") + 1e-2
        gx_1 = F.conv2d(
            x, weight_x1, bias=None, stride=1, padding=0, groups=1
        )
        gx_2 = F.conv2d(
            x, weight_x2, bias=None, stride=1, padding=0, groups=1
        )
        gy_1 = F.conv2d(
            x, weight_y1, bias=None, stride=1, padding=0, groups=1
        )
        gy_2 = F.conv2d(
            x, weight_y2, bias=None, stride=1, padding=0, groups=1
        )
